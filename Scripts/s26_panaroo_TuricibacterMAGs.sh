...
#===============================================================================
# File Name    : s26_panaroo_TuricibacterMAGs.sh
# Description  : Runs the panaroo pangenome tool on genomes annotated by bakta
# Usage        : sbatch s26_panaroo_TuricibacterMAGs.sh
...
#SBATCH --job-name=panaroo_Turicibacter
#SBATCH --cpus-per-task=8
#SBATCH --mem=8G
#SBATCH --output=slurm_out/panaroo/x_panaroo_Turicibacter_%A.out
#SBATCH --error=slurm_out/panaroo/y_panaroo_Turicibacter_%A.err

eval $( spack load --sh py-panaroo )

basedir="$PWD"
indir="${basedir}/d25_bakta/Turicibacter"
outdir="${basedir}/d26_panaroo/Turicibacter"
