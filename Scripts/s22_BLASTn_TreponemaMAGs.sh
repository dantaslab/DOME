...
#SBATCH --job-name=BLASTn_Treponema
#SBATCH --array=1-152%20
#SBATCH --cpus-per-task=1
#SBATCH --mem=2G
#SBATCH --output=slurm_out/blastn_MAGs/x_blastn_Treponema_%a.out
#SBATCH --error=slurm_out/blastn_MAGs/y_blastn_Treponema_%a.err
...
outdir="${basedir}/d22_BLASTn_MAGs/Treponema/BLASTn"
...
do
  ...
  subject=`sed -n ${i}p ${basedir}/Mapping_v7_TreponemaMAGs.txt`
  echo "Blasting ${sample} with ${subject}"
  time blastn \
          -query ${indir}/${sample}.fa \
          -db ${basedir}/d22_BLASTn_MAGs/Treponema/db/${subject}/${subject} \
          -outfmt 6 \
          -perc_identity 99 \
